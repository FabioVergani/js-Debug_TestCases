function isObject(x){return typeof x==='object'?x===null?0:1:0};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CaseGroups=(function(indefinito,Empty_arr,Simple_arr,Complex_arr,Empty_obj,Simple_obj,Complex_obj,Empty_str,Blank_str){
	return {
      Array:[
			Empty_arr,
			Simple_arr,
	...
	if(i){
		...
		console.groupCollapsed('%c -'+s,'font:11px arial;color:'+c,': #'+i);
		 g.forEach(function(x){
			 console.log('%c '+typeof(x),z,JSON.stringify(x),'\t\t',x);
			});
		console.groupEnd();
	};
};
...
for(var found in FinalResults){
	...
	for(var g in a){
		var b=a[g];
		console.group('%c'+g,'font-size:11px;color:'+Cat.col+';background:'+Cat.bgcol+';');
